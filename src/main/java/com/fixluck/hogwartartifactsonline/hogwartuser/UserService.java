...
public class UserService implements UserDetailsService {
    ...
    public HogwartsUser save(HogwartsUser newHogwartsUser) {
        //We need to encode plain password before saving to DB!!!
    ...
    //We do not using this update to change user password
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return this.userRepository.findByUsername(username)    // First, we need to find this user from database
                .map(hogwartsUser -> new MyUserPrincipal(hogwartsUser)) // If found, wrap the returned user instance in a
                                                                        // MyUserPrincipal
