...
class ArtifactControllerTest {
    ...
    void testSaveArtifactSuccess() throws Exception {
        //Given:
        //gỉa lập data đc front-end cung cấp từ controller
        ...
        //convert artifactdto sang json string vì ko thể gửi 1 dto object qua requestbody
        String json = this.objectMapper.writeValueAsString(artifactDto);

        //khởi tạo 1 fake input data để add artifact
        Artifact saveArtifact = new Artifact();
        saveArtifact.setId("1250808601744904197");
        saveArtifact.setName("Remembrall");
        saveArtifact.setDescription("A Remembrall was a magical large marble-sized glass ball that contained smoke which turned red when its owner or user had forgotten something. It turned clear once whatever was forgotten was remembered.");
        saveArtifact.setImageUrl("ImgUrl");

        given(this.artifactService.save(Mockito.any(Artifact.class))).willReturn(saveArtifact);

        //When and then: so sánh String json đc giả lập với json đc add vào từ artifact
    ...
    void testUpdateSuccess() throws Exception {
        ...
        updatedArtifact.setImageUrl("ImgUrl");

        given(this.artifactService.update(eq("1250808601744904197"), Mockito.any(Artifact.class))).willReturn(updatedArtifact);

        //When and then
        this.mockMvc.perform(MockMvcRequestBuilders.put(this.baseUrl +"/artifacts/1250808601744904197")
                .contentType(MediaType.APPLICATION_JSON).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
    ...
    @Test
    void updateErrorWithNonExistedId() throws Exception {
        //Given
        ArtifactDTO artifactDto = new ArtifactDTO(
                "1250808601744904197",
                "Remembrall",
    ...
    void testDeleteArtifactSuccess() throws Exception {
        //Given
        doNothing().when(this.artifactService).delete("1250808601744904197");

        //When and then
        this.mockMvc.perform(MockMvcRequestBuilders.delete(this.baseUrl +"/artifacts/1250808601744904197")
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
    ...
    @Test
    void testDeleteArtifactErrorWithNoExistedId() throws Exception {
        //Given
        doThrow(new ArtifactNotFoundException("1250808601744904197"))
                .when(this.artifactService).delete("1250808601744904197");
